...
Map.prototype.addPartsToArea = function(x, y, val, solid) {
    ...
    for (var i = -dx; i <= dx; i++) {
        for (var j = -dx; j <= dx; j++) {
            ...
            if (isValid(x + i, y + j, this.height, this.width)) {
                if (!this.isMirror(x + i, y + j, this.height, this.width, this.mirrored)) {
...
Map.prototype.cleanArchonSpawn = function(archonLocs) {
    for (var idx = 0; idx < archonLocs.length; idx++) {
        ...
        for (var i = -1; i <= 1; i++) {
            for (var j = -1; j <= 1; j++) {
                if (isValid(loc.x + i, loc.y + j, this.height, this.width)) {
                    if (!this.isMirror(loc.x + i, loc.y + j, this.height, this.width, this.mirrored)) {
...
Map.prototype.decorateDens = function(dens) {
    for (var idx = 0; idx < dens.length; idx ++) {
        ...
        for (var i = -3; i <= 3; i++) {
            for (var j = -3; j <= 3; j++) {
                if (Math.random() < .7 && isValid(den.x + i, den.y + j, this.height, this.width)) {
                    if (!this.isMirror(den.x + i, den.y + j, this.height, this.width, this.mirrored)) {
...
Map.prototype.toJSON = function(name) {
    return {
        "object-stream": [
            {
                "game-map": [
...
Map.prototype.save = function(name, dir) {
    var mapString = xml(this.toJSON(name), true);
